...
void encode(int process_id, int number_of_characters, int socket);
void thread(int socket, int process_id);

/* MAIN FUNCTION */
int main(int argc, char *argv[]){
  if (argc != 2) {
    printf("Usage: %s ENCRYPTION_PORT\n", argv[0]);
    exit(1);
  }
...
void thread(int socket, int process_id){
  ...
  int number_of_characters = getFile(socket, process_id);

  char confirm[2];
  bzero(confirm, 1);
  sprintf(confirm, "1"); // send the confirmation

  n = write(socket, confirm, strlen(confirm));

  getKey(socket, process_id);

  encode(process_id, number_of_characters, socket);
